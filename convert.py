...
def get_tissue_regions(sl, x, y, area_thresh=50000):
...
'''
def adjust_padding(bbox_oi, binary, pad=100):
    """Adjust padding based on image boundaries"""
    p,q = binary.shape
...
#def load_and_process_annotations(nm, x, y, bbox_oi, slide_id, channel_names):
def load_and_process_annotations_all_regions(nm, x, y, all_bboxes_scaled, slide_id, channel_names):
    ...
    ontology_names = ['0001225','0000074','0009773','0001637','0006851','0005215']
    
    # LOAD CORTEX
    mask = xml_to_mask(nm,(0,0),(x,y),[1])
    mask = mask.astype(np.uint8)
    #cortex = apply_bounding_box(mask, bbox_oi)
    ...
    mask = mask.astype(np.uint8)
    #multic = apply_bounding_box(mask, bbox_oi)
...
def main():
    ...
    # Configuration
    wsi_ext = ['.svs','.scn']
    channel_names = ['cortex','glomeruli','tubules','arteries-arterioles','peritubular_capillaries','IFTA']
    ontology_names = ['0001225','0000074','0009773','0001637','0006851','0005215']
    ...
    # Load QC data
    #qc_data, qc_slides, section, notes, double_slides = load_qc_data(qc_file)
